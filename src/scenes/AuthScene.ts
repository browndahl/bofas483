...
export class AuthScene extends Phaser.Scene {
  ...
  async create() {
    ...
    crisp(this.add.text(width / 2 - cardWidth / 2 + 28, height / 2 - 190, 'WITNESS IDENTITY', { fontFamily: DISPLAY_FONT, fontSize: '16px', color: '#f7bd62', letterSpacing: 1.2 }));
    const user = await authService.currentUser();
    crisp(this.add.text(width / 2 - cardWidth / 2 + 28, height / 2 - 155, !backendConfigured ? 'CLOUD OFFLINE / local saves remain active' : user?.is_anonymous ? 'GUEST SIGNAL / link without losing this habitat' : user ? `CONNECTED / ${user.email ?? 'OAUTH IDENTITY'}` : 'NO IDENTITY / guest play remains available', { fontFamily: UI_FONT, fontStyle: 'bold', fontSize: '12px', color: '#a8cdbb', wordWrap: { width: cardWidth - 56 } }));
    ...
    const status = crisp(this.add.text(width / 2, height / 2 + 72, '', { fontFamily: UI_FONT, fontStyle: 'bold', fontSize: '12px', color: '#ff8fcf', align: 'center', wordWrap: { width: cardWidth - 50 } })).setOrigin(0.5);
    ...
    const close = crisp(this.add.text(width / 2 + cardWidth / 2 - 24, height / 2 - 196, '×', { fontFamily: UI_FONT, fontStyle: 'bold', fontSize: '28px', color: '#a8cdbb' })).setOrigin(1, 0).setInteractive({ useHandCursor: true }); close.on('pointerup', () => this.scene.stop());
