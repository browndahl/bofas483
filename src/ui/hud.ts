...
import { crisp, fittedButtonFontSize, UI_FONT } from './typography';
...
export function button(scene: Phaser.Scene, x: number, y: number, width: number, height: number, label: string, color = 0x7af6bd) {
  ...
  const text = crisp(scene.add.text(2, 0, label, {
    fontFamily: UI_FONT,
    fontStyle: 'bold',
    fontSize: `${fittedButtonFontSize(label, width)}px`,
    color: Phaser.Display.Color.IntegerToColor(color).rgba,
    align: 'center',
    letterSpacing: width < 90 ? 0.1 : 0.35,
    wordWrap: { width: Math.max(24, width - 16), useAdvancedWrap: true },
    maxLines: height >= 40 ? 2 : 1
  })).setOrigin(0.5).setName('button-label');
